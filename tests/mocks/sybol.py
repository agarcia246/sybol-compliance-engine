...
class MockSybolClient:
    ...
    def fail_next(self, error: SybolAPIError | None = None) -> MockSybolClient:
        ...
        self._fail_queue.append(
            error or SybolAPIError("Sybol service unavailable", 503)
        )
        return self
    ...
    def _sign(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        signed.setdefault(
            "credentialSchema", {"id": self.schema_ref, "type": "JsonSchema"}
        )
        ...
        digest = hashlib.sha256(
            (str(signed.get("id")) + self.issuer_did).encode("utf-8")
        ).hexdigest()
        signed["proof"] = {
